            ...
            <div className="relative px-4 md:px-10 lg:px-20 py-24 z-10">
                <div className="max-w-[1400px] mx-auto space-y-32">
                    {p.chapters.map((chapter: { title: string; heroes: any[] }, chapterIdx: number) => (
                        <section key={chapterIdx} className="space-y-12">
                            <motion.div 
                            ...
                            >
                                <div className="w-2 h-10 bg-[#ee7c7e] rounded-full" />
                                <h2 className="text-3xl md:text-4xl font-black text-[#1a2355] dark:text-white tracking-tight uppercase">
                                    {chapter.title}
                                </h2>
                            </motion.div>
                                ...
                                {chapter.heroes.map((hero: { name: string; rank?: string; specialty?: string; description?: string; image?: string; date?: string }, i: number) => (
                                    ...
                                    >
                                        ...
                                        <div className="relative p-1.5 rounded-full border-2 border-dashed border-[#ee7c7e]/30 group-hover:border-[#ee7c7e] transition-all duration-500 group-hover:scale-105">
                                            <div className="w-32 h-32 rounded-full bg-gray-50 dark:bg-slate-700 overflow-hidden shadow-inner flex items-center justify-center text-[#1a2355]/20 group-hover:bg-[#ee7c7e]/10 transition-all duration-500 relative">
                                                {hero.image ? (
                                                    <Image 
                                                        src={hero.image} 
                                                        alt={hero.name} 
                                            ...
                                            <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-[#1a2355] text-white rounded-full flex items-center justify-center shadow-lg border-2 border-white dark:border-slate-800">
                                                <MilitaryTechIcon sx={{ fontSize: 18 }} />
                                            </div>
                                        </div>

                                        <div className="flex-1 space-y-4">
                                            <div className="space-y-1">
                                                <h3 className="font-black text-[#1a2355] dark:text-white text-lg leading-tight group-hover:text-[#ee7c7e] transition-colors duration-300">
                                                    {hero.name}
                                                </h3>
                                                {hero.rank && (
                                                    <div className="inline-block px-3 py-1 rounded-full bg-[#ee7c7e]/10 text-[#ee7c7e] text-[9px] font-bold uppercase tracking-widest">
                                                        {hero.rank}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            {hero.specialty && (
                                                <div className="flex items-center justify-center gap-2 text-xs text-[#1a2355]/60 dark:text-white/40 font-medium">
                                        ...
                                        {hero.date && (
                                            <div className="w-full pt-6 border-t border-gray-100 dark:border-slate-700/50">
                                                <div className="flex items-center justify-center gap-2 text-[#1a2355] dark:text-white/60 font-bold text-xs">
                                                    <CalendarTodayIcon sx={{ fontSize: 14, color: "#ee7c7e" }} />
                                                    {hero.date}
                                                </div>
                                            </div>
                                        )}
