...
import { getFacultyEmployeeById } from "@/data/staticFaculties";
...
interface Props {
  params: Promise<{ facultyId: string; employeeId: string }>;
}

export default function FacultyEmployeeDetailPage({ params }: Props) {
  const { facultyId, employeeId } = use(params);
  const employee = getFacultyEmployeeById(Number(employeeId));
    ...
    return (
      <div className="space-y-6">
        <SectionBlock title="Əməkdaş tapılmadı" accent>
          ...
          <Link
            href={`/faculties/${facultyId}/haqqimizda/emekdaslar`}
