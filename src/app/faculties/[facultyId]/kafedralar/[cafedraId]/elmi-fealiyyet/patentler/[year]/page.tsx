...
export default function PatentlerYearPage({ params }: Props) {
  ...
  const patents = (data?.sections.patents?.items ?? []).filter((patent) =>
    isOther ? patent.year === null : patent.year === Number(year)
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <FacultyPanel
        title={currentLang === "az" ? `${heading} — Patentlər` : `${heading} — Patents`}
      ...
      >
        {loading ? (
          ...
          <EmptyState
            ...
            hint={
              currentLang === "az"
                ? "Bu il üzrə patent tapılmadı."
                : "No patents were found for this year."
            }
          />
        ) : (
