...
export default function AnnouncementDetailPage({
    ...
    const [related, setRelated] = useState<ApiAnnouncement[]>([]);
    ...
    useEffect(() => {
        fetch(`${API_BASE}/api/announcement/public/all?start=0&end=100`)
            .then((r) => r.json())
            .then((data) => {
                const all: ApiAnnouncement[] = data.announcements ?? [];
                const found = all.find((a) => a.id === parseInt(id));
    ...
    const { date, month, year } = announcement ? parseDate(announcement.created_at) : { date: "", month: "", year: "" };
        ...
        <>
            ...
            {!announcement && (
                <div className="flex items-center justify-center min-h-screen">
                    <p className="text-gray-400 font-semibold text-lg">Yüklənir...</p>
                </div>
            )}
