        ...
        <main className="min-h-screen bg-white dark:bg-[#0b1330] transition-colors duration-500">
            ...
            {data.sections.map((section: any, idx: number) => (
                <HomeStyleSection 
                    ...
                    title={section.title} 
                    dark={idx % 2 === 1}
                    watermark={idx === 0 ? "Points" : idx === 1 ? "Grades" : idx === 2 ? "Format" : "Rules"}
                >
                    <div className={`p-10 md:p-16 rounded-[3rem] border ${idx % 2 === 1 ? 'border-white/10 bg-white/5 backdrop-blur-xl' : 'border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-white/5'} shadow-2xl`}>
                        ...
                        {section.items && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {section.items.map((item: any, i: number) => (
                                    <div key={i} className="group p-8 rounded-3xl bg-white/5 border border-white/10 hover:border-[#ee7c7e]/50 transition-all duration-500">
                                        <div className="w-12 h-12 rounded-2xl bg-[#ee7c7e]/10 flex items-center justify-center text-[#ee7c7e] mb-6 group-hover:bg-[#ee7c7e] group-hover:text-white transition-all">
                                            <ListAltIcon />
                                        </div>
                                        <h4 className={`text-xl font-black mb-3 ${idx % 2 === 1 ? 'text-white' : 'text-[#1a2355] dark:text-white'} uppercase tracking-tight`}>{item.title}</h4>
                                        <p className="text-gray-500 dark:text-white/60 leading-relaxed font-medium">{item.description}</p>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {section.subContent && (
                            <div className="mt-12 p-8 rounded-3xl bg-[#ee7c7e]/5 border-l-4 border-[#ee7c7e] flex items-center gap-6">
                                <HelpOutlineIcon className="text-[#ee7c7e]" sx={{ fontSize: 32 }} />
                                <p className="text-[#1a2355] dark:text-white text-lg font-black italic">
                                    &quot;{section.subContent}&quot;
                                </p>
                            </div>
                        )}
            ...
            <section className="px-4 md:px-10 lg:px-20 py-24 bg-gray-50 dark:bg-[#0b1330]">
                <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="p-10 rounded-[3rem] bg-[#1a2355] text-white shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-all duration-500">
                         ...
                         <AssessmentIcon sx={{ fontSize: 48 }} className="text-[#ee7c7e] mb-8" />
                         <h4 className="text-2xl font-black uppercase tracking-tighter mb-4">{lang === 'az' ? 'Şəffaflıq' : 'Transparency'}</h4>
                         <p className="text-white/60 font-medium leading-relaxed">{lang === 'az' ? 'Bütün imtahanlar videoçəkiliş və mərkəzləşdirilmiş sistem vasitəsilə nəzarətdə saxlanılır.' : 'All exams are monitored via video recording and a centralized system.'}</p>
                    </div>
                    <div className="p-10 rounded-[3rem] bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-all duration-500">
                         <GavelIcon sx={{ fontSize: 48 }} className="text-[#ee7c7e] mb-8" />
                         <h4 className="text-2xl font-black text-[#1a2355] dark:text-white uppercase tracking-tighter mb-4">{lang === 'az' ? 'Ədalətlilik' : 'Fairness'}</h4>
                         <p className="text-gray-500 dark:text-white/40 font-medium leading-relaxed">{lang === 'az' ? 'Qiymətləndirmə meyarları əvvəlcədən təyin edilir və bütün tələbələr üçün eynidir.' : 'Assessment criteria are predefined and uniform for all students.'}</p>
                    </div>
                    <div className="p-10 rounded-[3rem] bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-all duration-500">
                         <HelpOutlineIcon sx={{ fontSize: 48 }} className="text-[#ee7c7e] mb-8" />
                         <h4 className="text-2xl font-black text-[#1a2355] dark:text-white uppercase tracking-tighter mb-4">{lang === 'az' ? 'Dəstək' : 'Support'}</h4>
                         <p className="text-gray-500 dark:text-white/40 font-medium leading-relaxed">{lang === 'az' ? 'Apellyasiya və müraciət prosesləri tələbələrin hüquqlarını qorumaq üçün sadələşdirilmişdir.' : 'Appeals and application processes are simplified to protect student rights.'}</p>
                    </div>
                </div>
            </section>
        </main>
