                ...
                {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                        {Array.from({ length: 8 }).map((_, i) => (
                            <div key={i} className="animate-pulse bg-white dark:bg-slate-800/50 backdrop-blur-md rounded-[14px] shadow-xl border border-gray-100 dark:border-slate-700 p-10 h-64" />
                        ))}
                    </div>
                ) : error ? (
                    <div className="text-center py-32 bg-white dark:bg-slate-800/50 backdrop-blur-md rounded-[14px] border-2 border-dashed border-red-100 dark:border-red-900/20 shadow-sm">
                        <p className="text-red-400 font-black uppercase tracking-widest text-sm">
                            {t.error}
                        </p>
                    </div>
                ) : cafedras.length === 0 ? (
                    <div className="text-center py-32 bg-white dark:bg-slate-800/50 backdrop-blur-md rounded-[14px] border-2 border-dashed border-gray-100 dark:border-slate-700 shadow-sm">
                        ...
                        {cafedras.map((cafedra, i) => {
                            ...
                            return (
                                ...
                                >
                                    <Link
                                        href={baseLink}
                                        className="group block relative h-full bg-white dark:bg-slate-800/50 backdrop-blur-md rounded-[14px] shadow-sm hover:shadow-[0_20px_50px_rgba(26,35,85,0.1)] border-2 border-transparent hover:border-[#ee7c7e]/20 p-8 transition-all duration-500 overflow-hidden"
                                    >
