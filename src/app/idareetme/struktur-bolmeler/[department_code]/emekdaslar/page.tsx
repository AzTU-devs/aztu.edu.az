...
export default function DepartmentStaffPage({ params }: Props) {
  ...
  const filtered = workers.filter((w) => {
    const fullName = `${w.first_name} ${w.last_name} ${w.father_name}`.toLowerCase();
    const duty = (w.duty || "").toLowerCase();
  ...
  return (
    <div className="space-y-6">
      <SectionBlock title={currentLang === "az" ? "Şöbə əməkdaşları" : "Department Staff"} accent>
        <p className="text-sm text-gray-500 dark:text-slate-400 mb-10 leading-relaxed max-w-2xl font-medium">
          {currentLang === "az" 
            ? "Şöbənin fəaliyyətində iştirak edən inzibati və texniki heyət haqqında məlumat." 
            : "Information about administrative and technical staff involved in the department's operations."}
        </p>
        ...
        {workers.length === 0 ? (
          ...
          <>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-20 bg-gray-50 dark:bg-slate-800/20 rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-slate-700 shadow-sm">
