...
import { getFacultyById, getCafedraById } from "@/data/staticFaculties";
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 transition-colors">
        ...
        <div className="bg-[#1a2355] px-4 md:px-10 lg:px-20 pt-40 pb-12">
          <nav className="flex items-center gap-1.5 text-white/50 text-sm mb-4 flex-wrap">
            ...
            <ChevronRightIcon sx={{ fontSize: 14 }} />
            <Link
              href={`/faculties/${facultyId}/haqqimizda`}
              className="hover:text-white transition-colors truncate max-w-[160px]"
            >
              {faculty?.short_name ?? faculty?.name ?? "Fakültə"}
            </Link>
            <ChevronRightIcon sx={{ fontSize: 14 }} />
            <Link
              href={`/faculties/${facultyId}/kafedralar`}
            ...
            <span className="text-white/80 truncate max-w-[160px]">
              {cafedra?.short_name ?? cafedra?.name ?? "Kafedra"}
            </span>
          </nav>

          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2 leading-tight">
            {cafedra?.name ?? "Kafedra"}
          </h1>
          {cafedra?.short_name && (
